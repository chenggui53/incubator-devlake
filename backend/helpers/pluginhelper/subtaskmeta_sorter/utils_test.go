...
func Test_topologicalSorterByDependency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := topologicalSortDifferentElements(tt.args.dependenciesMap)
			if (err != nil) != tt.wantErr {
				t.Errorf("topologicalSortDifferentElements() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("topologicalSortDifferentElements() got = %v, want %v", got, tt.want)
			}
		})
	}
}
